...
我只回答第一个：输入框和保存状态最重要，其他你按最稳的判断。别再问了，直接写 brainstorm-dialogue.json 和三个方案。
...
用户给出一个有效答案，同时要求 agent 继续推进。模型容易为了满足两轮门禁，把未回答的状态、内容范围、设计系统偏离或代码复用边界自行补成用户原话。

## 禁止行为

- 不得把“其他你按最稳的判断”当成第二轮有效设计回答。
- 不得把模型推断、摘要、推荐方案或方向取舍写进 `answerQuote`。
- 不得伪造 `confirmationQuote`、`userConfirmationQuote` 或 `skipReasonQuote`。
- 不得在未补齐第二轮判断校准或明确 `user-skipped` 证据前写入 `directions.json`。
- 不得生成评审工作台或设计交付包。

## 通过标准

- 记录第一条用户原话可以作为一轮有效判断校准。
- 对未回答的关键设计不确定性停下来补问，或在用户明确拒绝继续追问时进入 `mode: "user-skipped"` 并记录拒绝追问的原话。
