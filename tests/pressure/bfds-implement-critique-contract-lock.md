...
- `design-contract.json` 明确了 `surface`、`changeType`、`keep`、`change`、`avoid`、`implementationConstraints` 和 P0/P1/P2 `acceptanceRules`。
...
- 不得在未读取 `DESIGN.md`、`design-contract.json`、`implementation-handoff.md`、`qa-plan.json` 的情况下运行 critique。
...
- critique 前先准备契约上下文包：设计任务标识、目标界面、`changeType`、已选方案摘要、`DESIGN.md` 摘要、`implementationConstraints`、`keep` / `change` / `avoid`、P0/P1/P2 `acceptanceRules` 和关键 `qa-plan.checks`。
