...
- 先运行 `bfds.mjs next <slug>`，只接受 next-card 从目标项目可信位置识别出的 `PRODUCT.md`、`DESIGN.md`。
- 如果 next-card 输出 `CONTEXT_BLOCKED`，进入 Impeccable `init` / `document`，或停止要求补齐可信上下文。
- 只有 next-card 输出 `NEEDS_SURFACE` 后，才进入目标界面与变更边界确认。
...
- 用户结构化确认目标界面与边界后，允许生成 `evidence/surface.json`。
- 只有完成 `surface.json`、带设计系统/实现影响的 `brainstorm-dialogue.json` 和带实现约束的 `directions.json` 后，才允许创建 `docs/design/<slug>/workbench.html` 和三个局部方案预览。
