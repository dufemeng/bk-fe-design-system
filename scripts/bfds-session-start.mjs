...
- NEEDS_DIRECTIONS：父会话做苏格拉底式设计脑暴，一次只问一个设计表达问题。先写 evidence/brainstorm-dialogue.json；用户确认 2-3 个方向取舍后，才写 evidence/directions.json，然后重跑 gate。
- NEEDS_WORKBENCH：只根据方向证据生成 workbench.html 和 option-a/b/c.html。
- NEEDS_SELECTION：停止等待用户明确选择；推荐方案或“你定”不算选择。
- NEEDS_CONTRACT：向用户回显选择证据原话和摘要；用户确认后才写设计交付包。
