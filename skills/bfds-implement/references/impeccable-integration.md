...
- `vendor/impeccable/cli/engine/detect-antipatterns.mjs`
- `vendor/impeccable/skill/scripts/detect.mjs`
...
如果 vendor 依赖未安装或 Node 版本不满足 Impeccable 要求，停止并记录“Impeccable detect 未运行：入口或依赖未验证”。不要把未运行写成通过。

## Critique

`critique` 是设计评审增强：

1. 读取 `vendor/impeccable/skill/reference/critique.md`。
...
- `vendor/impeccable/skill/reference/live.md`
