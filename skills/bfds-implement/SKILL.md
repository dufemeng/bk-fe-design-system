...
1. 如果用户没有指定 `<slug>`，运行 `node <bfds-implement-skill-dir>/scripts/bfds-status.mjs --json --limit 5`，让用户选择。
2. 对目标设计任务运行：

```bash
node <bfds-implement-skill-dir>/scripts/bfds-gate.mjs <slug> --sync-status
```
...
```bash
node <bfds-implement-skill-dir>/scripts/bfds-gate.mjs <slug> --sync-status --mark implementing
node <bfds-implement-skill-dir>/scripts/bfds-gate.mjs <slug> --sync-status --mark implemented
node <bfds-implement-skill-dir>/scripts/bfds-gate.mjs <slug> --sync-status --mark qa-passed
```
