...
- `vendor/impeccable/package.json` 暴露 CLI bin：`cli/bin/cli.js`。
- `vendor/impeccable/cli/bin/cli.js` 支持 `detect` 和 `skills`。
- `vendor/impeccable/skill/SKILL.src.md` 的命令表包含 `init`、`document`、`critique`、`live`。
- `vendor/impeccable/skill/reference/init.md` 说明 `init` 会建立 `PRODUCT.md`、`DESIGN.md` 和 `.impeccable/live/config.json`。
- `vendor/impeccable/skill/reference/live.md` 是局部 live 变体协议。
...
2. 如果缺失，读取 `vendor/impeccable/skill/reference/init.md`。
...
1. 恢复当前 BFDS slug。
2. 读取 `vendor/impeccable/skill/reference/live.md`。
