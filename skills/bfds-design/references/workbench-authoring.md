...
生成任何 `workbench.html` 或 `option-a/b/c.html` 前，必须确认：

1. 设计上下文门禁已完成：`bfds-context.mjs` 返回 `CONTEXT_READY`，并记录可信 `PRODUCT.md` / `DESIGN.md` 路径。
2. 目标界面与变更边界门禁已完成：目标界面、现状来源、改动类型、必须保留、允许改变、必须避免都已确认。
3. 设计方向探索门禁已完成：A/B/C 三个三方向设计规格已产出并通过自检。
