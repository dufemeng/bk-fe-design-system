...
Workbench 是静态 HTML 审阅台，不是事件 server。用户在对话中回复“选 A/B/C”或合并意见。
...
- 三个 option 必须放在 iframe 中。
- iframe 内设计稿必须服从目标项目 `DESIGN.md`，不能继承 Kami 的纸感样式。
- option 文件应内联或链接自己的目标项目样式，不依赖 workbench.css。
...
允许在 option 内使用少量内联 JS 做本地状态切换，例如：
