...
| “你来选”“挑最稳的”“推荐一个并生成交付包” | 任意 | 任意 | 任意 | 可以推荐方案，但必须等待用户明确确认 A/B/C 或合并方案；不生成设计交付包 |
...
- `selected`：只有已有可引用的用户选择原话时，才生成设计交付包。
...
- 用户要求 agent 代选方案：只给推荐和理由，要求用户回复“选 A/B/C”或明确合并方案；不要写合同或更新状态。
