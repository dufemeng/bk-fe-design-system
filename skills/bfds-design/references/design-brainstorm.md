...
开始提问前先定位 grounding 证据：目标界面证据、`DESIGN.md` 具体 token/组件规则/小节、可复用源码文件或组件名。`codeReuseHypothesis` 中出现源码路径时，必须来自 `rg`、文件读取或等价源码定位；runtime 会拒绝不存在的 repo-relative 路径。找不到源码证据时要明说“暂无可验证源码复用证据”，不能把“复用现有组件”写成无来源假设。

不要问 API、数据模型、权限、商业模式、后端架构，也不要在本阶段重新确认目标界面。不要把原型里已经能看见的布局事实再问一遍，例如“按钮放左边还是右边”这类肉眼可见问题。

`answerQuote`、`userConfirmationQuote` 和 `skipReasonQuote` 必须逐字取自用户回复。用户只回答了部分问题时，先补问未回答项或把该项标为未确认；不得把模型推断、摘要或建议写成用户原话。
...
- “这个目标区域看起来是高频设置流，我判断用户更需要快速复核而不是探索。确认吗？这会让实现优先保留已由源码定位确认的设置面板表单节奏，并按 `DESIGN.md` 的 `spacing.sm/md` 调整输入区，而不是新增展示型卡片。”
...
`directions.json` 必须引用同目录的 `surface.json` 和 `brainstorm-dialogue.json`。下面示例假设列出的源码路径已在当前仓库中真实存在；如果找不到源码证据，`codeReuseHypothesis` 改写为“暂无可验证源码复用证据：...”而不是编造路径。
...
- A/B/C 都包含具体 `DESIGN.md` token/组件规则/小节引用、带真实存在源码路径的代码复用假设或“暂无可验证源码复用证据”说明、允许变更边界、实现风险和自审检查点。
- 至少一个方案覆盖关键状态或关键交互。
