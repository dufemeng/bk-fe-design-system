...
模型只提交当前阶段的用户回答、设计判断和确认原话；证据 JSON、`status.json`、设计交付包结构由 BFDS runtime 生成或校验。所有 `answerQuote`、`confirmationQuote`、`selectionQuote`、`skipReasonQuote` 和 `echoConfirmQuote` 必须来自用户原话，不得代写、润色、摘要改写或补全用户未回答的内容。

Claude Code 中，单选、多选和确认类用户输入必须用 `AskUserQuestion`。Impeccable init 阶段按 next-card 每轮成组询问 2-3 个项目级问题；其他开放式设计表达问题可以普通文本。
