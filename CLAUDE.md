...
- BFDS 是面向前端代码生成的设计规范执行层（Design System Execution Layer），不是产品规划、后端架构或通用工程实现框架。
- 当前主线以 `README.md`、两个 BFDS skill 协议和 `docs/plans/bfds-design-system-driven-codegen-plan.md` 为准：`DESIGN.md` 是唯一设计规范事实源，设计稿只是生码方向校准物，最终价值落在规范驱动生码、自审和局部微调闭环。
...
- `codeReuseHypothesis` 源码路径校验、`implementation-self-review.json`、`selfReviewNote`、`qa-report.md` 的 `Result` / `Evidence` 等执行细节，以当前 skill reference 和 `src/runtime/bfds/` 为准；不要凭本文件补写协议。
